...
| B-12 | **The evals have never been run.** The data and its shape-check shipped in 0.5.6; nothing has put the twenty queries or the six scenarios in front of a model, on any of the three tiers. `npm test` proves the evals are valid, never that the skills pass them | run 8 | 2 | 2 | 0 | 1.0 | open |
...
| B-11 | `test/evals/` (audit item 8) | Shipped in 0.5.6: 20 triggers with a fixed 60/40 split, 6 scenarios, a prompt-injection fixture, and 10 shape checks. Running them is `B-12` |
